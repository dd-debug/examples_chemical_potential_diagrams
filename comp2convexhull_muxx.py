...
for i in range(len(x)-2):
    ...
    print(triangle)
    pc=PatchCollection([polygon],alpha = 0.5, facecolor = colors[i],edgecolor = colors[i],linewidths=1)
    ax.add_collection(pc)
    ax.plot([1,1],[(y2-y1)/(x2-x1) * (1-x2) + y2,(y0-y2)/(x0-x2) * (1-x2) + y2],c = colors[i],linewidth = 3)
